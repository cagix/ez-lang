            ...
            switch (instruction) {
                case Instruction.Return returnInst -> {
                    if (returnInst.from instanceof Operand.ConstantOperand constantOperand) {
                        ...
                        execStack.stack[base] = execStack.stack[base+registerOperand.regnum];
                    }
                ...
                case Instruction.Move moveInst -> {
                    if (moveInst.to instanceof Operand.RegisterOperand toReg) {
                        if (moveInst.from instanceof Operand.RegisterOperand fromReg) {
                            execStack.stack[base + toReg.regnum] = execStack.stack[base + fromReg.regnum];
                        }
                        else if (moveInst.from instanceof Operand.ConstantOperand constantOperand) {
                            execStack.stack[base + toReg.regnum] = new Value.IntegerValue(constantOperand.value);
                        }
                        else throw new IllegalStateException();
                    }
                    else throw new IllegalStateException();
                ...
                case Instruction.ConditionalBranch cbrInst -> {
                    ...
                    if (cbrInst.condition instanceof Operand.RegisterOperand registerOperand) {
                        Value value = execStack.stack[base + registerOperand.regnum];
                ...
                case Instruction.Call callInst -> {
                    ...
                    for (Operand.RegisterOperand arg: callInst.args) {
                        execStack.stack[base + reg] = execStack.stack[base + arg.regnum];
                        reg += 1;
                    }
                    // Call function
                    ...
                    if (!(callInst.callee.returnType instanceof Type.TypeVoid)) {
                        execStack.stack[base + callInst.returnOperand.regnum] = execStack.stack[baseReg];
                    }
                }
                case Instruction.Unary unaryInst -> {
                    ...
                    Value unaryValue = execStack.stack[base + unaryOperand.regnum];
                    if (unaryValue instanceof Value.IntegerValue integerValue) {
                        switch (unaryInst.unop) {
                            case "-": execStack.stack[base + unaryInst.result.regnum] = new Value.IntegerValue(-integerValue.value); break;
                            // Maybe below we should explicitly set Int
                            case "!": execStack.stack[base + unaryInst.result.regnum] = new Value.IntegerValue(integerValue.value==0?1:0); break;
                ...
                case Instruction.Binary binaryInst -> {
                    ...
                    if (binaryInst.left instanceof Operand.ConstantOperand constant)
                        ...
                        x = ((Value.IntegerValue) execStack.stack[base + registerOperand.regnum]).value;
                    else throw new IllegalStateException();
                    if (binaryInst.right instanceof Operand.ConstantOperand constant)
                        ...
                        y = ((Value.IntegerValue) execStack.stack[base + registerOperand.regnum]).value;
                    else throw new IllegalStateException();
                    ...
                    execStack.stack[base + binaryInst.result.regnum] = new Value.IntegerValue(value);
                }
                case Instruction.NewArray newArrayInst -> {
                    execStack.stack[base + newArrayInst.destOperand.regnum] = new Value.ArrayValue(newArrayInst.type);
                }
                case Instruction.NewStruct newStructInst -> {
                    execStack.stack[base + newStructInst.destOperand.regnum] = new Value.StructValue(newStructInst.type);
                }
                case Instruction.AStoreAppend arrayAppendInst -> {
                    Value.ArrayValue arrayValue = (Value.ArrayValue) execStack.stack[base + arrayAppendInst.array.regnum];
                    if (arrayAppendInst.value instanceof Operand.ConstantOperand constant) {
                        ...
                        arrayValue.values.add(execStack.stack[base + registerOperand.regnum]);
                    }
                    else throw new IllegalStateException();
                }
                case Instruction.ArrayStore arrayStoreInst -> {
                    if (arrayStoreInst.arrayOperand instanceof Operand.RegisterOperand arrayOperand) {
                        Value.ArrayValue arrayValue = (Value.ArrayValue) execStack.stack[base + arrayOperand.regnum];
                        int index = 0;
                        if (arrayStoreInst.indexOperand instanceof Operand.ConstantOperand constant) {
                            ...
                            Value.IntegerValue indexValue = (Value.IntegerValue) execStack.stack[base + registerOperand.regnum];
                        ...
                        if (arrayStoreInst.sourceOperand instanceof Operand.ConstantOperand constantOperand) {
                            ...
                            value = execStack.stack[base + registerOperand.regnum];
                        }
                        else throw new IllegalStateException();
                ...
                case Instruction.ArrayLoad arrayLoadInst -> {
                    if (arrayLoadInst.arrayOperand instanceof Operand.RegisterOperand arrayOperand) {
                        ...
                        if (arrayLoadInst.indexOperand instanceof Operand.ConstantOperand constant) {
                            execStack.stack[base + arrayLoadInst.destOperand.regnum] = arrayValue.values.get((int) constant.value);
                        }
                        else if (arrayLoadInst.indexOperand instanceof Operand.RegisterOperand registerOperand) {
                            Value.IntegerValue index = (Value.IntegerValue) execStack.stack[base + registerOperand.regnum];
                            execStack.stack[base + arrayLoadInst.destOperand.regnum] = arrayValue.values.get((int) index.value);
                        }
                        else throw new IllegalStateException();
                    } else throw new IllegalStateException();
                }
                case Instruction.SetField setFieldInst -> {
                    if (setFieldInst.structOperand instanceof Operand.RegisterOperand structOperand) {
                        Value.StructValue structValue = (Value.StructValue) execStack.stack[base + structOperand.regnum];
                ...
                case Instruction.GetField getFieldInst -> {
                    if (getFieldInst.structOperand instanceof Operand.RegisterOperand structOperand) {
                        ...
                        execStack.stack[base + getFieldInst.destOperand.regnum] = structValue.fields[index];
                    } else throw new IllegalStateException();
                }
